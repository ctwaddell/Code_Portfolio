...
extern "C"
{
    ...
    //r and c are by value, so you cant 'change' them.
    void DLL_EXPORT RPG(int rowInput, int columnInput, char* seedInput, char* output, int outputSize)
    {
        rows = rowInput;
        columns = columnInput;
        std::hash<std::string> hasher;
        std::srand(hasher(seedInput));
        ...
        snprintf(output, outputSize, "{\n"
            ...
            "\t\"bounty\": %d\n"
            "}\n", 
            board.c_str(), solution.c_str(), seedInput, bounty);
    }
}